...
export function MessageItem({ message, isMe, onDelete }: MessageItemProps) {
  ...
  return (
    <div className={`flex items-start group ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
      ...
      <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[75%] px-1`}>
        <div className="flex items-center space-x-2">
          {isMe && message.id && (
            <Button 
              ...
              className="h-7 w-7 p-0 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-all rounded-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm cursor-pointer"
              title="Hapus Pesan"
            >
          ...
          <div className={`rounded-2xl transition-shadow group-hover:shadow-sm ${
            isMe 
          ...
          } ${isImageUrl(message.content) ? 'p-1.5' : 'px-4 py-2'}`}>
            ...
            <p className={`text-[9px] mt-1 text-right flex items-center justify-end leading-none font-medium ${isMe ? 'text-indigo-200' : 'text-slate-400'} ${isImageUrl(message.content) ? 'px-2 pb-1' : ''}`}>
              {displayTime}
              {isMe && (
